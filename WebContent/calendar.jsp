...
//	달력이 최초로 실행되면 이전페이지가 존재하지 않기 때문에 year와 month가 null 이므로 NumberFormatException을 처리해야 한다.
	try{
		year = Integer.parseInt(request.getParameter("year"));
		month = Integer.parseInt(request.getParameter("month"));
		
//		month에 13이 넘어왔으면 달력에는 다음해 1월을 표시해야 하고 month에 0이 넘어왔으면 전년도 12월을 표시해야 한다.		
		if (month >= 13) {
			year++;
			month = 1;
		} else if (month <= 0) {
...
<table width="700" align="center" border="1" cellpadding="5" cellspacing="2">
	...
	<tr>
		<th>
			<!-- 
				...
				href 속성에 id가 지정되면 같은 문서에서 id가 지정된 요소로 이동하고 url(주소)이 지정되면 url로 지정된 페이지로 이동한다.
	...
	<tr>
<% 
//	1일이 출력될 위치를 맞추기위해서 1일의 요일만큼 반복하며 빈 칸을 출력한다.
...
//	빈 칸에 출력할 전달 날짜의 시작일을 계산한다.
	int start = 0;
	if (month == 1){
//		start = MyCalendar.lastDay(year - 1, 12) - MyCalendar.weekDay(year, month, 1);			// 1월
		start = 31 - MyCalendar.weekDay(year, month, 1);			// 1월
	} else {
		start = MyCalendar.lastDay(year, month - 1) - MyCalendar.weekDay(year, month, 1);		// 2월 ~ 12월
	}

//	1일이 출력될 위치를 맞추기위해서 1일의 요일만큼 반복하며 빈 칸에 전달 날짜를 출력한다.
	for (int i = 1; i <= MyCalendar.weekDay(year, month, 1); i++){
		if(i == 1){
			out.println("<td class=startSun>" + (month == 1 ? 12 : month - 1) + "/" + ++start + "</td>");
		} else {
			out.println("<td class=startEtc>" + (month == 1 ? 12 : month - 1) + "/" + ++start + "</td>");
		}
	}
...
//	1일 부터 달력을 출력할 달의 마지막 날짜까지 반복하며 날짜를 출력한다.
	for (int i = 1; i <= MyCalendar.lastDay(year, month); i++){
		...
		boolean flag = false;
		int child = 0;
//		어린이날이 토요일 또는 일요일과 겹치는 경우 그 다음 첫 번째 비공휴일을 대체 공휴일로 한다.
...
//		양력 공휴일을 표시한다.
		if (month == 1 && i == 1){
			...
			out.println("<td class='holiday'>" + i + "<br><span>어린이날<br>대체 공휴일</span></td>");
		} else {
//			공휴일이 아니면 요일별로 글자색을 다르게 지정하기 위해서 <td> 태그에 요일에 따른 class 속성을 지정한다. (if로 해도되고 switch도 됨)
...
//	다음달 1일의 요일을 계산한다.
	int last = 0;
	if (month == 12) {
		last = 7 - MyCalendar.weekDay(year + 1, 1, 1);
	} else {
		last = 7 - MyCalendar.weekDay(year, month + 1, 1);
	}
	
//	날짜를 다 출력하고 남은 빈 칸에 다음달 날짜를 다음달 1일의 요일부터 토요일까지 반복하며 출력한다.
	if(MyCalendar.weekDay(year, month + 1, 1) != 0){
		for (int i = 1; i <= last; i++){
			if(i == last){
				out.println("<td class='lastSat'>" + i + "</td>");
			} else {
				out.println("<td class='lastEtc'>" + i + "</td>");
			}
		}
	}
	...
	if(week != 0){
		...
		for (int i = week; i <= 6; i++){
			if(i == 6){
				...
				out.println("<td class='afterEtc'>" + (month == 12 ? 1 : month + 1) + "/" + ++start + "</td>");
			}
		}
